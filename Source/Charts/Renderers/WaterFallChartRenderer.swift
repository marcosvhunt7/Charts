...
open class WaterFallChartRenderer: LineScatterCandleRadarRenderer
{
    @objc open weak var dataProvider: WaterFallChartDataProvider?
    
    @objc public init(dataProvider: WaterFallChartDataProvider?, animator: Animator?, viewPortHandler: ViewPortHandler?)
    {
    ...
    {
        guard let dataProvider = dataProvider, let waterFallData = dataProvider.waterFallData else { return }
//ICandleChartDataSet
        for set in waterFallData.dataSets as! [IWaterFallChartDataSet]
        {
            if set.isVisible
            {
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: IWaterFallChartDataSet)
    {
        guard let
        ...
        {
            // get the entry
            guard let e = dataSet.entryForIndex(j) as? WaterFallChartDataEntry else { continue }
    ...
    open override func drawValues(context: CGContext)
    {
        guard
            ...
            let waterFallData = dataProvider.waterFallData,
            let animator = animator
            else { return }
        ...
        {
            var dataSets = waterFallData.dataSets
            ...
            {
                guard let dataSet = dataSets[i] as? IBarLineScatterCandleBubbleChartDataSet else {
                    continue
                }
                ...
                for j in stride(from: _xBounds.min, through: _xBounds.range + _xBounds.min, by: 1)
                {
                    guard let e = dataSet.entryForIndex(j) as? WaterFallChartDataEntry else { break }
                    
                    pt.x = CGFloat(e.x)
                    pt.y = CGFloat(e.close * phaseY)
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        {
            guard
                let set = waterFallData.getDataSetByIndex(high.dataSetIndex) as? IWaterFallChartDataSet,
                set.isHighlightEnabled
                else { continue }
            
            guard let e = set.entryForXValue(high.x, closestToY: high.y) as? WaterFallChartDataEntry else { continue }
            ...
            let lowValue = e.open * Double(animator.phaseY)
            let highValue = e.close * Double(animator.phaseY)
